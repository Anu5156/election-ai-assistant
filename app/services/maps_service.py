...
def get_polling_stations(location: str, bias_lat: Optional[float] = None, bias_lng: Optional[float] = None) -> List[Dict[str, Any]]:
    """
    Searches for polling stations using Google Places (New) API.
    """
    try:
        ...
        payload = {"textQuery": f"polling station near {location}, India"}
        if lat and lng:
            payload["locationBias"] = {"circle": {"center": {"latitude": lat, "longitude": lng}, "radius": 5000.0}}
...
def get_route_details(origin_lat: float, origin_lng: float, dest_lat: float, dest_lng: float) -> Tuple[str, str, str]:
    ...
    Returns (duration_text, distance_text, polyline).
    """
    try:
